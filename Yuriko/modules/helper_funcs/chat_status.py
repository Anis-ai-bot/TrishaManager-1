...
def is_user_admin(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    if (
        ...
        or user_id in [2023126723, 1087968824]
    ...
    def is_dev_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if not user:
            ...
            update.effective_message.reply_text(
                "Ye command kewal apne developer bhaiya ke liye hai."
                " Tera liye nahi.",
            )
...
def sudo_plus(func):
    ...
    def is_sudo_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if not user:
            ...
            update.effective_message.reply_text(
                "Bhai,Bhen jo bhi ho apke pass ye right na ha sorry aur so sameless you are saste admin  🙂 dil se bura laga support @Fun_Games_Chat",
            )
    ...
    def is_whitelist_plus_func(
        ...
        **kwargs,
        ...
        if user and is_whitelist_plus(chat, user.id):
            return func(update, context, *args, **kwargs)
        update.effective_message.reply_text(
            f"Tera pass ye sab dekhne ka access na ha sed lyf.\nVisit @{SUPPORT_CHAT}",
        )
...
def bot_can_delete(func):
    ...
    def delete_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_delete = "Bro/sis delete message ka permission to do yaar."
        else:
            cant_delete = f"I can't delete messages in <b>{update_chat_title}</b>!\nBro/sis delete message ka permission to do yaar."
...
def can_promote(func):
    ...
    def promote_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_promote = "Ma kise ko nahi kara skti promote/demote yaha per!\nMake sure Na ma admin hu🥺 na he add Admin permission."
        else:
            cant_promote = (
...
def can_restrict(func):
    ...
    def restrict_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_restrict = "Na Ma kise ko restrict kar skti here!\nMake sure Admin na hu na iseliya."
        else:
...
def user_can_ban(func):
    ...
    def user_is_banhammer(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if (
            ...
            and user not in [2023126723, 1087968824]
        ):
            update.effective_message.reply_text(
                "Sorry bete, but you're not worthy to wield the banhammer👉😏.",
            )
            return ""
        return func(update, context, *args, **kwargs)
